    ...
    for (const line of lines) {
        ...
        if (trimmed.startsWith('//') && trimmed.includes('DD')) {
            ...
            if (ddMatch) {
                ...
                if (dispMatch) {
                    dataset.disposition = dispMatch[1].split(',').map((d) => d.trim());
                }
            }
        }
...
function analyzeGovernance(parsed) {
    const findings = [];
            ...
            return {
                content: [{
                        ...
                        text: JSON.stringify({
                            ...
                            dataLineage: {
                                flows: parsed.steps.map((step) => ({
                                    step: step.name,
                                    program: step.program,
                                    inputs: step.datasets.filter((ds) => ds.disposition?.includes('OLD') || ds.disposition?.includes('SHR')),
                                    outputs: step.datasets.filter((ds) => ds.disposition?.includes('NEW') || ds.disposition?.includes('MOD'))
                                }))
                            },
                            performance: {
