...
function parseIdcamsDefine(defineStatement) {
    ...
    for (const line of lines) {
        ...
        // Determine file type
        if (upper.includes('INDEXED'))
            fileType = 'KSDS';
        else if (upper.includes('NONINDEXED'))
            fileType = 'ESDS';
        else if (upper.includes('NUMBERED'))
            fileType = 'RRDS';
        else if (upper.includes('LINEAR'))
            fileType = 'LDS';
        // Parse RECORDSIZE
        ...
        if (upper.includes('RECOVERY'))
            attributes.recovery = 'RECOVERY';
        if (upper.includes('UNIQUEKEY'))
            attributes.uniqueKey = true;
        if (upper.includes('UPGRADE'))
...
function parseCobolFd(fdStatement) {
...
export const vsamMapper = {
    ...
    handler: async (args) => {
        try {
            ...
            const piiFields = result.fields?.filter((f) => f.isPii) || [];
