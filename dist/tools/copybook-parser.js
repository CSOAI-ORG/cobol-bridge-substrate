...
function parsePicClause(pic) {
    ...
    if (pattern.includes('A'))
        return { type: 'ALPHABETIC', length };
    if (pattern.includes('S'))
        return { type: 'SIGNED', length };
    return { type: 'UNKNOWN', length };
}
function detectPii(fieldName, picClause) {
...
function parseCopybook(copybook) {
    ...
    for (const line of lines) {
        ...
        if (piiInfo.isPii) {
            field.compliance = COMPLIANCE_MAPPINGS[piiInfo.piiType] || ['GDPR'];
            field.governanceTags = ['PII', 'SENSITIVE'];
        }
        fields.push(field);
...
export const copybookParser = {
    ...
    handler: async (args) => {
        try {
            ...
            // Count PII fields
            const piiFields = parsed.fields.filter((f) => f.isPii);
            return {
                content: [{
                        type: 'text',
                        text: JSON.stringify({
                            ...
                            summary: {
                                ...
                                complianceFrameworks: [...new Set(piiFields.flatMap((f) => f.compliance || []))]
                            },
                            performance: {
