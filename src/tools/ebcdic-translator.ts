...
function unpackPackedDecimal(bytes: number[]): { value: string; sign: string } {
  if (bytes.length === 0) return { value: '', sign: '+' };
...
export const ebcdicTranslator = {
  ...
  handler: async (args: z.infer<typeof EbcdicTranslatorSchema>) => {
    try {
