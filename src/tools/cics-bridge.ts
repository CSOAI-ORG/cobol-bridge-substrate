...
function assessSecurityRisk(region: string, transactions: string[]): any {
  const risks = [];
...
function assessCompliance(region: string, transactions: string[]): any {
  const findings: any[] = [];
...
export const cicsBridgeAssessment = {
  ...
  handler: async (args: z.infer<typeof CicsBridgeSchema>) => {
    try {
